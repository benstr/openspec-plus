...
Codex uses `.codex/skills` for a fresh repository. If `.agents/skills` already exists and
`.codex/skills` does not, the repository has selected the shared agents convention: install Codex
skills into `.agents/skills` and do not recreate `.codex/skills`. If both roots exist, prefer the
official `.codex/skills` target to avoid silently changing an ambiguous repository. Commands remain
global Codex prompts per the pinned 0.2.0 contract.
...
- `test/install.test.mjs`: the zero-dependency smoke suite (node:test). Scenarios: dry-run
  ...
  standard `.codex/skills` plus existing-repository `.agents/skills` target selection;
